...
router.post("/",
  ...
  async (req, res) => {
    ...
    const { categories, latitude, longitude, excludeHistories = 1 } = req.body;
    ...
    try {
      let result = await fetchRestaurant(categories, latitude, longitude, offset)
      ...
      for (let i = 0; i < 5; i++) {
        ...
        if (result.data.businesses.length && !historiesIndex[result.data.businesses[0].id]) {
          ...
          result = await fetchRestaurant(categories, latitude, longitude, offset);
